...
'''
Livelock
- multiple threads or processes are actively responding to each other
  to solve conflict, but that prevent them from making progress.
- often cause by algorithms that are intended to recover from deadlock.
- To avoid them, ensure that only one thread takes action to solve the conflict
  choosing by priority or other mechanism like random selection.
'''
